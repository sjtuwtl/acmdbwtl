...
public class BTreeFile implements DbFile {
	...
	private BTreeLeafPage findLeafPage(TransactionId tid, HashMap<PageId, Page> dirtypages, BTreePageId pid, Permissions perm,
		...
		if (pid.pgcateg() == BTreePageId.LEAF)
			...
			page = getPage(tid, dirtypages, pid, Permissions.READ_WRITE);
			BTreeEntry i = null;
			Iterator<BTreeEntry> iter = ((BTreeInternalPage) page).iterator();
			while (iter.hasNext()) {
				i = iter.next();
				if (f == null || f.compare(Op.LESS_THAN_OR_EQ, i.getKey()))
					return findLeafPage(tid, dirtypages, i.getLeftChild(), perm, f);
			}
			return findLeafPage(tid, dirtypages, i.getRightChild(), perm, f);
		}
		else
			throw new DbException("It's not internal or leaf pages.");
	}
	...
	protected BTreeLeafPage splitLeafPage(TransactionId tid, HashMap<PageId, Page> dirtypages, BTreeLeafPage page, Field field)
		...
		for (int i = page.getNumTuples() / 2; i > 0; i--) {
			...
			newPage.insertTuple(tmp);
		}

		newPage.setRightSiblingId(page.getRightSiblingId());
		newPage.setLeftSiblingId(page.getId());
		if (page.getRightSiblingId() != null)
			((BTreeLeafPage) getPage(tid, dirtypages, page.getRightSiblingId(), Permissions.READ_WRITE)).setLeftSiblingId(newPage.getId());
		page.setRightSiblingId(newPage.getId());

		BTreeEntry newEntry = new BTreeEntry(tmp.getField(keyField), page.getId(), newPage.getId());
		BTreeInternalPage parent = getParentWithEmptySlots(tid, dirtypages, page.getParentId(), tmp.getField(keyField));
		newEntry.setRecordId(newPage.iterator().next().getRecordId());
		parent.insertEntry(newEntry);
		updateParentPointers(tid, dirtypages, parent);

		if (field.compare(Op.LESS_THAN_OR_EQ, tmp.getField(keyField)))
			return page;
		else
			return newPage;
	}
	...
	protected BTreeInternalPage splitInternalPage(TransactionId tid, HashMap<PageId, Page> dirtypages,
		...
		BTreeEntry curEntry = null, LeftEntry = null;
		for (int i = (page.getNumEntries() + 1) / 2; i > 0; i--) {
			curEntry = iter.next();
			page.deleteKeyAndRightChild(curEntry);
			if (LeftEntry != null)
				newPage.insertEntry(LeftEntry);
			if (i > 1)
				LeftEntry = curEntry;
		}

		LeftEntry.setLeftChild(curEntry.getRightChild());
		newPage.updateEntry(LeftEntry);
		curEntry.setLeftChild(page.getId());
		curEntry.setRightChild(newPage.getId());


		BTreeInternalPage parent = getParentWithEmptySlots(tid, dirtypages, page.getParentId(), curEntry.getKey());
		curEntry.setRecordId(newPage.iterator().next().getRecordId());
		parent.insertEntry(curEntry);
		updateParentPointers(tid, dirtypages, page);
		updateParentPointers(tid, dirtypages, newPage);

		if (field.compare(Op.LESS_THAN, curEntry.getKey()))
			return page;
		else
			return newPage;
	}
	...
	protected void stealFromLeafPage(BTreeLeafPage page, BTreeLeafPage sibling,
		...
		int left = page.getNumTuples();
		int right = sibling.getNumTuples();
		if (!isRightSibling) {
			Iterator<Tuple> iter = sibling.reverseIterator();
			for (int i = 0; i < (right - left) / 2; i++) {
				Tuple tmp = iter.next();
				sibling.deleteTuple(tmp);
			...
			entry.setKey(iter.next().getField(keyField));
		}
	...
	protected void stealFromLeftInternalPage(TransactionId tid, HashMap<PageId, Page> dirtypages,
		...
		// some code goes here
		int left = page.getNumEntries();
		int right = leftSibling.getNumEntries();
		Iterator<BTreeEntry> iter = leftSibling.reverseIterator();
		for (int i = 0; i < (right - left) / 2; i++ ) {
			BTreeEntry tmp = iter.next();
			leftSibling.deleteKeyAndRightChild(tmp);

			page.insertEntry(new BTreeEntry(parentEntry.getKey(), tmp.getRightChild(), page.getChildId(0)));

			parentEntry.setKey(tmp.getKey());
		}
	...
	protected void stealFromRightInternalPage(TransactionId tid, HashMap<PageId, Page> dirtypages,
		...
		BTreeEntry l = null;
		int left = page.getNumEntries();
		int right = rightSibling.getNumEntries();
		for (int i = 0; i < (right - left) / 2; i++ ) {
			BTreeEntry tmp = iter.next();
			rightSibling.deleteKeyAndLeftChild(tmp);

			if (l == null) {
				l = new BTreeEntry(parentEntry.getKey(), page.reverseIterator().next().getRightChild(), tmp.getLeftChild());
			}
			else
				l = new BTreeEntry(parentEntry.getKey(), l.getRightChild(), tmp.getLeftChild());
			page.insertEntry(l);
	...
	protected void mergeLeafPages(TransactionId tid, HashMap<PageId, Page> dirtypages,
		...
		while (iter.hasNext()) {
			Tuple tmp = iter.next();
			rightPage.deleteTuple(tmp);
			leftPage.insertTuple(tmp);
		}
	...
	protected void mergeInternalPages(TransactionId tid, HashMap<PageId, Page> dirtypages,
		...
		// Delete the entry in the parent corresponding to the two pages that are merging -
		// deleteParentEntry() will be useful here
		int right = rightPage.getNumEntries();
		Iterator<BTreeEntry> iter = rightPage.iterator();
		BTreeEntry newParent = new BTreeEntry(parentEntry.getKey(), leftPage.reverseIterator().next().getRightChild(), rightPage.iterator().next().getLeftChild());
		newParent.setRecordId(parentEntry.getRecordId());
		leftPage.insertEntry(newParent);
		for (int i = 0; i < right; i++) {
			BTreeEntry e = iter.next();
			rightPage.deleteKeyAndRightChild(e);
	 ...
	 * @see #handleMinOccupancyPage(TransactionId, HashMap, BTreePage)
	 */
	public synchronized ArrayList<Page> deleteTuple(TransactionId tid, Tuple t)
			throws DbException, IOException, TransactionAbortedException {
		HashMap<PageId, Page> dirtypages = new HashMap<PageId, Page>();
	...
	BTreeRootPtrPage getRootPtrPage(TransactionId tid, HashMap<PageId, Page> dirtypages) throws DbException, IOException, TransactionAbortedException {
		...
		return (BTreeRootPtrPage) getPage(tid, dirtypages, BTreeRootPtrPage.getId(tableid), Permissions.READ_WRITE);
	}
...
class BTreeFileIterator extends AbstractDbFileIterator {
	...
	public void open() throws DbException, TransactionAbortedException {
		BTreeRootPtrPage rootPtr = (BTreeRootPtrPage) Database.getBufferPool().getPage(
				tid, BTreeRootPtrPage.getId(f.getId()), Permissions.READ_WRITE);
		BTreePageId root = rootPtr.getRootId();
		curp = f.findLeafPage(tid, root, Permissions.READ_WRITE, null);
		it = curp.iterator();
	}
...
class BTreeSearchIterator extends AbstractDbFileIterator {
	...
	public void open() throws DbException, TransactionAbortedException {
		...
		if(ipred.getOp() == Op.EQUALS || ipred.getOp() == Op.GREATER_THAN
			...
			curp = f.findLeafPage(tid, root, Permissions.READ_WRITE, ipred.getField());
		}
		else {
			curp = f.findLeafPage(tid, root, Permissions.READ_WRITE, null);
		}
